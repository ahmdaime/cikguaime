/**
 * IDME PRO License System - FULLY AUTOMATED (FIXED VERSION)
 * Version 3.3.0 - With Robust Validation & Date Parsing
 * By CikguAimeDotCom
 *
 * FEATURES:
 * - License validation API with IMPROVED date parsing ✅
 * - Device fingerprinting and tracking ✅
 * - AUTO license key generation ✅
 * - AUTO email delivery to customer ✅
 * - Purchase form handling ✅
 * - Receipt upload to Google Drive ✅
 * - Admin notifications ✅
 *
 * FIXES in v3.3.0:
 * - ✅ Robust date parsing (handles Date objects, DD/MM/YYYY, YYYY-MM-DD)
 * - ✅ Flexible status checking (trims whitespace, case-insensitive)
...
/**
 * Handle GET requests - License Validation
 */
function doGet(e) {
  try {
...
/**
 * Handle POST requests - Purchase Form Submission
 */
function doPost(e) {
  try {
    ...
    sheet.appendRow([
      ...
      data.phone,                    // C: Phone
      data.email,                    // D: Email
      formatDate(expiryDate),        // E: Expiry Date (DD/MM/YYYY)
      'ACTIVE',                      // F: Status
...
<body>
  <div class="container">
    ...
    <div class="footer">
      ...
      <p style="margin: 5px 0; font-size: 12px;">IDME PBD Helper v3.3.0 - Anti-Sharing Protection</p>
    </div>
  </div>
</body>
</html>
...
// ============================================================================
// LICENSE VALIDATION - IMPROVED WITH ROBUST DATE PARSING
// ============================================================================
  ...
  try {
    ...
    if (dateStr.match(/^\d{1,2}\/\d{1,2}\/\d{4}$/)) {
      ...
      if (isNaN(parsed.getTime())) {
        throw new Error('Invalid DD/MM/YYYY date');
      }
      return parsed;
    }
    ...
    if (dateStr.match(/^\d{1,2}-\d{1,2}-\d{4}$/)) {
      ...
      const parsed = new Date(year, month, day);
      if (isNaN(parsed.getTime())) {
        throw new Error('Invalid MM-DD-YYYY date');
      }
      return parsed;
    }
...
/**
 * Normalize status field (trim whitespace, uppercase) - FIXED VERSION
 */
...
/**
 * Validate license with device tracking - IMPROVED VERSION
 */
function validateLicenseWithDevice(licenseKey, deviceId, deviceName) {
  try {
    ...
    Logger.log('Validating license: ' + normalizedKey);
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      if (key === normalizedKey) {
        Logger.log('License found at row ' + (i + 1) + ' for: ' + customerName);

        // Check status (normalized comparison) - FIXED
        ...
        // Parse dates with improved parsing - FIXED
        const today = new Date();
        ...
        try {
          ...
          Logger.log('Expiry date parsed successfully: ' + formatDate(expiry));
        } catch (dateError) {
          Logger.log('Date parsing failed for row ' + (i + 1) + ': ' + dateError.message);
          Logger.log('Expiry date value: ' + expiryDate + ' (type: ' + typeof expiryDate + ')');
        ...
        Logger.log('✅ Validation successful! Days left: ' + daysLeft);
    ...
    Logger.log('License key not found: ' + normalizedKey);
...
/**
 * Format date as DD/MM/YYYY - FIXED VERSION
 */
...
function extendLicense(licenseKey, months = 1) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (data[i][0] === licenseKey) {
      const currentExpiry = parseDate(data[i][4]);
      const today = new Date();
...
/**
 * Test validation with all licenses - DEBUGGING FUNCTION
 */
function testAllLicenses() {
  try {
    const sheet = getActiveSheet();
    const data = sheet.getDataRange().getValues();

    Logger.log('========================================');
    Logger.log('🧪 TESTING ALL LICENSES');
    Logger.log('========================================\n');
